...
- **由谁启动**：**只能被 LLM 客户端拉起**。做法是在 Claude Desktop / Cursor / VS Code 等桌面客户端的 MCP 配置里写 `"command": "dmcp"`，由客户端自己把 dmcp 启动起来、并接管它的输入/输出。**你不需要、也不能在终端里手动启动它**——它随客户端启动而生、随客户端退出而灭。
...
- **是什么**：stdio 与 HTTP 同时开启——一个进程，两套入口。
- **由谁启动**：stdio 那端由 LLM 客户端拉起，HTTP 那端由你手动启动。
- **适用场景**：既想在本机桌面端「省 token」，又想让浏览器 / 云端 / 手机也能用同一套上游工具。
