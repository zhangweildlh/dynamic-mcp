...
- 一个 HTTP 端口同一时刻只能被一个进程真正用起来。如果你不小心启动了两个、且都指向同一个端点，后启动的那个要么连不上、要么把前一个顶掉，既浪费资源又容易让你困惑（"我到底连的是哪一个？"）。
- 所以 dynamic-mcp 在启动时就把它探明，并替你做最合理的处理，而不是等出事再报错。

### 四种情形与处理结果

| 你新启动的 | 同端口已有的 | 处理结果（大白话） |
| --- | --- | --- |
| 纯 `http` | `both`（已含 http） | 新开的纯 http **没意义**（已有的 both 已经能提供 http），**8 秒后自动退出**，让你直接复用已有的 both。 |
| 纯 `http` | 纯 `http` | 同上：新开的属于"双开浪费"，**8 秒后自动退出**，复用已有的那个。 |
| `both` | 纯 `http`（**未**加 `--no-evict`） | 新 `both` **接管**端口：stdio 立即可用，8 秒后把 HTTP 端口接过来（期间先把旧的纯 http 关掉）。一个进程顶两个用，最省。 |
| `both` | 纯 `http`（**已**加 `--no-evict`） | 不抢端口：新 `both` **只开 stdio**，HTTP 不启用；旧的纯 http 继续占着端口，两者和平共存（但你得接受"新 both 没 HTTP"）。 |
| `both` | `both` | 同理视为"双开浪费"：新 `both` **只开 stdio**，HTTP 不启用，不关旧的。 |

> 关键提醒：**`--no-evict` 这个开关只看"已有的那个纯 http"有没有加**。新 `both` 加不加 `--no-evict` 不影响判断——决定要不要强杀旧 http 的，是旧 http 自己启动时有没有写 `--no-evict`。
...
1. **先想清楚是否真要两个实例**——多数情况直接复用已有的即可，不必再开。
2. **想让旧的纯 http 不被强杀**：在启动那个纯 http 时加 `--no-evict`；这样将来你开 `both` 就不会接管端口，而是只跑 stdio。
3. **看到「双开浪费」或「端口被占用」提示别慌**：日志里会有两层提示——一层是"双开浪费"的 `warn`，一层是"端口被占 + 该改成哪个端口"的 `warn`（两层都会弹窗/打印，但合并成一个弹窗）。按提示把启动命令和 LLM 配置里的端口改一致即可。
4. **弹窗只是提醒**：Windows 会弹一个可手动关闭的真实对话框，Linux 走 `notify-send`，macOS 走 `osascript`；看完关掉即可，不影响服务运行。

> 💡 检测只针对 HTTP 端点。**纯 stdio 模式启动的实例之间互不影响**——stdio 不占端口，所以多个 stdio 实例可以并行存在（它们本就只被各自的 LLM 客户端接管）。
